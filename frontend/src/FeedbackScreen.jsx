  ...
  const handleRecord = async () => {
    try {
      ...
      if (!isRecording) {
        ...
        if (audioData) {
          const id = await dbService.createFeedback({ duration: audioData.duration }, audioData.blob);
  ...
  return (
    <div className="h-screen bg-white flex flex-col">
      ...
      <div className="border-b border-gray-200 p-6 flex items-center gap-4">
        ...
        <h1 className="text-2xl font-light text-gray-900">Feedback</h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="p-8 flex flex-col items-center gap-3">
          <button
            ...
            `}
            title={isRecording ? 'Stop' : 'Record feedback'}
          >
            {isRecording ? (
              <>
            ...
            {inProgress.map(item => (
              <div key={item.id} className="mb-8 pb-8 border-b border-gray-200 last:border-b-0">
                ...
                {item.status === 'ready_for_review' && (
                  <>
                    <p className="text-gray-900 mb-4">{item.transcript}</p>
