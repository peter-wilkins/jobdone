...
The contract is the canonical JobDone object graph, not HTTP JSON itself. JSON is the MVP transport codec for readability, but API code should keep transport encoding behind a small Codec Adapter so a denser codec can be introduced later without changing Zod schemas or product objects.
...
CI should also include a database conformance check against a disposable JobDone schema. That check should fail if JobDone-owned sync tables drift from the canonical contract: wrong schema, snake_case JobDone identifiers, text owner IDs where Supabase Auth UUIDs are expected, missing UUID sync IDs, missing Sync Transaction fields, missing scoped association FKs, or exposed RLS policy mistakes.
...
Property tests are the proof loop for these contracts. Preferred test slices are backend API plus real Postgres, IndexedDB plus frontend sync adapter, and later a full app wake-up sync harness. Playwright button pushing is not the main way to prove sync correctness.
