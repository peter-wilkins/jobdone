...
function locationContentHash(location = {}) {
  return stableHash({
    ...
    accuracyMeters: roundedCoordinate(location.accuracyMeters ?? location.accuracy_meters ?? location.accuracy),
...
export function toCanonicalLocationRecord(location = {}) {
  ...
  return {
    ...
    accuracyMeters: normalized?.accuracyMeters ?? location.accuracyMeters ?? location.accuracy_meters ?? location.accuracy ?? null,
  ...
  if (sameId) {
    ...
    const { data, error } = await jobdoneDb
      ...
      .update({
        ...
        accuracyMeters: location.accuracyMeters ?? sameId.accuracyMeters ?? null,
