...
**Pre-Extraction**:
A behind-the-scenes extraction pass run before Confirmation to help JobDone make good Context Clue guesses from the raw Capture, nearby device/platform evidence, and existing user data. Pre-Extraction is advisory: it can suggest or rank review options, but it is not saved as the final Entry structure. Pre-Extraction should be debounced, local-only, and deterministic-first in MVP: keyword matching, existing candidate names, and cheap rules should be tried before any LLM fallback.
...
**Server-Readable Team Recall**:
A later opt-in mode for large Team or enterprise Owner Scopes where complete local recall becomes impractical on ordinary devices. In this mode, a Team may deliberately run storage and Recall in a server-readable environment, ideally self-hosted or enterprise-controlled, so central search/indexing can operate over Team content. This is not the default privacy model and should require clear Team-level consent, admin control, and product copy that explains the backend can read that Team's payloads.
_Avoid_: Silent Enterprise Indexing, Accidental Team Decryption, Personal Data Search Escape Hatch
...
- PostGIS is a spatial query layer for Locations, not a Location identity rule. It can power nearby, within-radius, clustering, route-planning, and map views, while proximity remains evidence for suggestions rather than automatic Location collapse.
- The app/API Location shape should stay simple for Google Maps and phone integrations: `latitude`, `longitude`, and optional `accuracyMeters`. The database may store the coordinate as a PostGIS `geography(Point, 4326)` value and convert at the adapter seam, so PostGIS internals and longitude-first point syntax do not leak into app-facing payloads.
- Location geometry is point-with-optional-accuracy-radius for MVP. Exact addresses, GPS captures, remembered places, farms, sites, and neighbourhood-like places all start as a point plus `accuracyMeters`. Polygons, bounds, and multi-point areas are deferred until real map workflows prove they are needed.
...
- Server-Readable Team Recall is a later enterprise/large-Team mode, not the default. It may be appropriate when Team data exceeds practical local device restore/search limits and the Team deliberately chooses self-hosted or enterprise-controlled server-readable storage.
